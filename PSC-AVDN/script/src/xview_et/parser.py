...
def parse_args():
    ...
    parser.add_argument(
        "--root_dir",
        type=str,
        default=str(Path(__file__).resolve().parents[4] / "datasets"),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--ignoreid", type=int, default=-100, help="ignoreid for action"
    )
    ...
    parser.add_argument(
        "--darknet_model_file",
        type=str,
        default="./datasets/AVDN/pretrain_weights/yolo_v3.cfg",
    )
    parser.add_argument(
        "--darknet_weight_file", type=str, default="./AVDN/pretrain_weights/best.pt"
    )
    ...
    parser.add_argument(
        "--num_replacement", "-num_replacement", action="store_true", default=False
    )
    parser.add_argument(
        "--train_val_on_full", "-train_val_on_full", action="store_true", default=False
    )
    parser.add_argument(
        "--act_pred_token",
        default="ob_txt",
        choices=["ob", "ob_txt", "ob_hist", "ob_txt_hist"],
    )
    ...
    parser.add_argument(
        "--optim", type=str, default="rms", choices=["rms", "adam", "adamW", "sgd"]
    )
