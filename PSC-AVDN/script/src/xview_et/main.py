...
def train(
    ...
    if args.eval_first:
        ...
        if args.train_val_on_full:
            for env_name, env in val_full_traj_envs.items():
                ...
                for metric, val in score_summary.items():
                    ...
                    writer.add_scalar(
                        "%s/%s" % (metric, env_name), score_summary[metric], 0
                    )
        else:
        ...
        for env_name, env in val_envs.items():
            ...
            agent.env = env
            loader = torch.utils.data.DataLoader(env, batch_size=1)
            agent.test(loader, feedback="teacher")
            preds = agent.get_results()
    ...
    best_val = {
        "val_unseen": {"spl": 0.0, "state": ""},
        "val_unseen_full_traj": {"spl": 0.0, "state": ""},
    }
    ...
    for idx in range(start_iter, start_iter + args.iters, interval):
        ...
        agent.train(loader, args.log_every, feedback=args.feedback, nss_w_weighting=1)
        ...
        for env_name, env in val_envs.items():
            ...
            for metric, val in score_summary.items():
                loss_str += ", %s: %.2f" % (metric, val)
                writer.add_scalar(
                    "%s/%s" % (metric, env_name), score_summary[metric], iter
                )
            if env_name in best_val:
                if score_summary["spl"] >= best_val[env_name]["spl"]:
                    ...
                    agent_eval.save(
                        iter, os.path.join(args.ckpt_dir, "best_%s" % (env_name))
                    )
        for env_name, env in val_envs.items():
            env_name += "_human_att"
            agent_eval.env = env
        ...
        write_to_record_file(
            (
                "%s (%d %d%%) %s"
                % (
                    timeSince(start, float(iter) / args.iters),
                    iter,
                    float(iter) / args.iters * 100,
                    loss_str,
                )
            ),
            record_file,
        ...
        for env_name in best_val:
            write_to_record_file(
                env_name + " | " + best_val[env_name]["state"], record_file
            )
...
def valid(args, val_envs, val_full_traj_envs, rank=-1):
    ...
    for env_name, env in val_envs.items():
        ...
        pred_results = agent.get_results()
        if "val_unseen" in env_name:
            print("inference_result on val_unseen is generated.")
            np.save("./output_vai_unseen_result.npy", pred_results)
        else:
            score_summary, result = env.eval_metrics(pred_results)
...
def main():
    ...
    train_env, train_full_traj_env, val_envs, val_full_traj_envs = build_dataset(
        args, rank=rank
    )
