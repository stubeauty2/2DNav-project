...
class EncoderVL(nn.Module):
    def __init__(self, args):
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            args.demb, args.encoder_heads, args.demb, args.dropout_transformer_encoder,
        )
    ...
    def forward(self, emb_lang, emb_frames, emb_directions, lengths):
        length_max = np.max(lengths)
        length_lang = emb_lang.shape[1]
        length_mask_pad = length_lang + length_max * 2
        mask_pad = torch.zeros(
            (len(emb_lang), length_mask_pad), device=emb_lang.device
        ).bool()
        ...
        emb_all = self.encode_inputs(
            emb_lang, emb_frames, emb_directions, length_lang, mask_pad
        )
        mask_attn = model_util.generate_attention_mask(
            length_lang, length_max, emb_all.device,
        )
        output = self.enc_transformer(
            emb_all.transpose(0, 1), mask_attn, mask_pad
        ).transpose(0, 1)
        return output, mask_pad

    def encode_inputs(
        self, emb_lang, emb_frames, emb_directions, lengths_lang, mask_pad
    ):
