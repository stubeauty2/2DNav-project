...
class PosLearnedEncoding(nn.Module):
    ...
    def forward(self, lang, frames, directions, lens_lang, lens_frames):
        ...
        pos_frames = torch.stack(
            [torch.arange(0, frames.shape[1]) + l for l in lens_lang]
        )
        pos_directions = torch.stack(
            [torch.arange(0, directions.shape[1]) + l for l in lens_lang]
        )
...
class TokenLearnedEncoding(nn.Module):
    ...
    def forward(self, lang, frames, directions):
        token_lang = (
            torch.ones(lang.shape[:2], device=lang.device, dtype=torch.long) * 0
        )
        token_lang_emb = self.emb(token_lang)
        lang += token_lang_emb
        token_frames = (
            torch.ones(frames.shape[:2], device=frames.device, dtype=torch.long) * 1
        )
        token_frames_emb = self.emb(token_frames)
        frames += token_frames_emb
        token_directions = (
            torch.ones(directions.shape[:2], device=directions.device, dtype=torch.long)
            * 2
        )
